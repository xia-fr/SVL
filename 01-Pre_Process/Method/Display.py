...
def renderData(filename=''):
    ...
    if Entities['Nodes'] and Entities['Elements']:
        ...
        Paraviewfile.write("<DataArray type=\"Float32\" Name=\"Vs\" format=\"ascii\">\n%s" % head4)
        for eTag in Entities['Elements']:
            mTag = Entities['Elements'][eTag]['attributes']['material']
            if 'E' in Entities['Materials'][mTag]['attributes']:
                Vs = np.sqrt(Entities['Materials'][mTag]['attributes']['E']/(2*Entities['Materials'][mTag]['attributes']['rho']*(1+Entities['Materials'][mTag]['attributes']['nu'])))
            else:
                Vs = 10
            Paraviewfile.write("%E " % Vs)
