...
Element("EQlin2DQuad4", nodes, 8, VTK_LINEAR_QUAD, GROUP_ELEMENT_QUAD), t(th), cf1(cf1), cf2(cf2), zref(zref), Type(Type) {
    //The element nodes.
    theNodes.resize(4);
...
EQlin2DQuad4::UpdateState(){
    ...
    for(unsigned int k = 0; k < wi.size(); k++){
        ...
        Eigen::VectorXd EqLinParam = ComputeGGmaxDamping(vs,z,rho,gamma);
...
void 
EQlin2DQuad4::SetDomain(std::map<unsigned int, std::shared_ptr<Node> > &nodes){
...
void 
EQlin2DQuad4::SetDamping(const std::shared_ptr<Damping> &damping){
    //The damping model
    theDamping = damping;
}
...
EQlin2DQuad4::GetTotalDegreeOfFreedom() const{
...
Eigen::MatrixXd 
EQlin2DQuad4::GetStrainRate() const{
    //number of integration points.
    unsigned int nPoints = QuadraturePoints->GetNumberOfQuadraturePoints();
...
Eigen::VectorXd 
EQlin2DQuad4::ComputeStrain(const Eigen::MatrixXd &Bij) const{
    //Gets the element coordinates in deformed configuration. 
    ...
    Eigen::VectorXd nodalDisplacement;
    nodalDisplacement.resize(8);
...
Eigen::MatrixXd 
EQlin2DQuad4::ComputeJacobianMatrix(const double ri, const double si) const{
...
Eigen::MatrixXd 
EQlin2DQuad4::ComputeShapeFunctionMatrix(const double ri, const double si) const{
...
Eigen::MatrixXd 
EQlin2DQuad4::ComputeStrainDisplacementMatrix(const double ri, const double si, const Eigen::MatrixXd &Jij) const{
...
Eigen::VectorXd
EQlin2DQuad4::ComputeGGmaxDamping(const double vs, const double z, const double rho, const double gamma) const{
    ...
    if (Type=="DARENDELI") {
        GGmax  = 1.0/(1.0+pow(gamma/gammar,phi(4)));
