...
Recorder::WriteResponse(std::shared_ptr<Mesh> &mesh, unsigned int step){
    ...
    if(strcasecmp(Name.c_str(),"PARAVIEW") == 0){
        ...
        WriteElementResponse(mesh, step);
    }
    else if(strcasecmp(Name.c_str(),"SECTION") == 0){
...
//Writes state variable data to the file.
void 
Recorder::WriteElementResponse(std::shared_ptr<Mesh> &mesh, unsigned int step){
    //Gets node and element information from the mesh.
    std::map<unsigned int, std::shared_ptr<Element> > Elements = mesh->GetElements();
    ...
    // Only does this once, for the last time step
    // Last time step index is stepTotal - 2 because of indexing from 0 and because
    // the first time step is the "0.00" time step in the simulation (doesn't count)
    else if( (strcasecmp(Response.c_str(),"MAXSTRAIN") == 0 ) && (step == (stepTotal-2))){
        for(unsigned int k = 0; k < IDs.size(); k++){
            //Gets the material strain.
            double MaxStrain = Elements[IDs[k]]->GetMaxStrain(); 
    
            //Writes the max strain value
            OutputFile << IDs[k] << " " << MaxStrain << " \n";
        }
    }
}
