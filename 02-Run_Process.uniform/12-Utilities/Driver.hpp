...
UpdateElements(std::shared_ptr<Mesh> &theMesh, RSJresource& jsonFile){
    ...
    for(unsigned int k = 0; k < Tags["add"].size(); k++){
        ...
        if(strcasecmp(Name.c_str(),"lin2DTruss2") == 0){
            ...
            double thickness = jsonFile["Elements"][eTag]["attributes"]["th"].as<double>();
            unsigned int matID = jsonFile["Elements"][eTag]["attributes"]["material"].as<int>();
            std::string eType = jsonFile["Elements"][eTag]["attributes"]["type"].as<std::string>("DARENDELI");
            unsigned int nGauss = jsonFile["Elements"][eTag]["attributes"]["np"].as<int>(4);
            std::string Quadrature = jsonFile["Elements"][eTag]["attributes"]["rule"].as<std::string>("GAUSS");

            //Instantiate the TIEQlin2DQuad4 element.
            theElement = std::make_shared<TIEQlin2DQuad4>(nodes, theMesh->GetMaterial(matID), thickness, Quadrature, nGauss, eType, zref, eref);
        }
...
UpdateDampings(std::shared_ptr<Mesh> &theMesh, RSJresource& jsonFile){
    ...
    for(unsigned int k = 0; k < Tags["add"].size(); k++){
        ...
        else if(strcasecmp(Name.c_str(),"Uniform") == 0){
            parameters.resize(2);
            parameters[0] = jsonFile["Dampings"][dTag]["attributes"]["z0"].as<double>(0.0);
            parameters[1] = jsonFile["Dampings"][dTag]["attributes"]["dt"].as<double>(0.0);

            unsigned int nfilts = jsonFile["Dampings"][dTag]["attributes"]["freqs"].size();
            filterFreqs.resize(nfilts);
            filterCoeffs.resize(nfilts);
            for(unsigned int k = 0; k < nfilts; k++){
                filterFreqs[k] = jsonFile["Dampings"][dTag]["attributes"]["freqs"][k].as<double>(0.0);
                filterCoeffs[k] = jsonFile["Dampings"][dTag]["attributes"]["coeffs"][k].as<double>(0.0);
            }
        }
        ...
        std::shared_ptr<Damping> theDamping = std::make_shared<Damping>(Name, parameters, filterFreqs, filterCoeffs);
