...
TIEQlin2DQuad4::CommitState(){
    ...
    for(unsigned int k = 0; k < nPoints; k++){
        ...
        if (theDamping[k]) theDamping[k]->CommitDamping();
...
TIEQlin2DQuad4::ReverseState(){
    ...
    for(unsigned int k = 0; k < nPoints; k++){
        ...
        if (theDamping[k]) theDamping[k]->ReverseDamping();
    }
}
...
TIEQlin2DQuad4::InitialState(){
    ...
    for(unsigned int k = 0; k < nPoints; k++){
        ...
        if (theDamping[k]) theDamping[k]->InitialDamping(3);
    }
...
TIEQlin2DQuad4::UpdateState(){
    ...
    for(unsigned int k = 0; k < wi.size(); k++){
        ...
        if (theDamping[k]) {
            theDamping[k]->SetDampingRatio(EqLinParam(1));
            Eigen::VectorXd Stress = theMaterial[k]->GetStress();
            theDamping[k]->UpdateDamping(Stress);
        }
    }
}
...
TIEQlin2DQuad4::SetDamping(const std::shared_ptr<Damping> &damping){
    //Copy for each gauss point
    unsigned int nPoints = QuadraturePoints->GetNumberOfQuadraturePoints();
    theDamping.resize(nPoints);
    for(unsigned int i = 0; i < nPoints; i++){
        theDamping[i] = damping->CopyDamping();
        theDamping[i]->InitialDamping(3);
    }
}
...
TIEQlin2DQuad4::GetStress() const{
    ...
    for(unsigned int i = 0; i < wi.size(); i++){
        ...
        if(theDamping[i]) {
            theDamping[k]->SetDampingRatio(EqLinParam(1));
            Cij *= theDamping[i]->GetStiffnessMultiplier();
        } 
        */
    }
...
TIEQlin2DQuad4::ComputeStiffnessMatrix(){
    ...
    for(unsigned int i = 0; i < wi.size(); i++){
        ...
        Eigen::MatrixXd Cij = theMaterial[i]->GetTangentStiffness();
        if(theDamping[i]) {
            theDamping[i]->SetDampingRatio(EqLinParam(1));
            Cij *= theDamping[i]->GetStiffnessMultiplier();
        }
...
TIEQlin2DQuad4::ComputeInternalForces(){
    ...
    for(unsigned int i = 0; i < wi.size(); i++){
        ...
        if (theDamping[i]) Stress += theDamping[i]->GetDampingForce();
