...
ZeroLength1D::CommitState(){
    ...
    if (theDamping) theDamping->CommitDamping();
}
...
ZeroLength1D::ReverseState(){
    ...
    if (theDamping) theDamping->ReverseDamping();
}
...
ZeroLength1D::InitialState(){
    ...
    if (theDamping) theDamping->InitialDamping(1);
}
...
ZeroLength1D::UpdateState(){
    ...
    if (theDamping) {
        ...
        theDamping->UpdateDamping(stress);
    }
}
...
ZeroLength1D::SetDamping(const std::shared_ptr<Damping> &damping){
    ...
    // Initialize damping variables
    theDamping->InitialDamping(1);
}
...
ZeroLength1D::ComputeInternalForces(){
    ...
    Eigen::VectorXd stress = theMaterial->GetStress();
    if (theDamping) {
        stress += theDamping->GetDampingForce();
    } 
